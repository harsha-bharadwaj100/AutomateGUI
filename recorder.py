...
def on_move(x, y):
    if len(recording) >= 1:
        if (
            recording[-1]["action"] == "pressed"
            and recording[-1]["button"] == "Button.left"
        ) or (
            recording[-1]["action"] == "moved"
            and time.time() - recording[-1]["_time"] > 0.02
        ):
...
def on_click(x, y, button, pressed):
    ...
    if len(recording) > 1:
        if (
            recording[-1]["action"] == "unclicked"
            and recording[-1]["button"] == "Button.right"
            and recording[-1]["_time"] - recording[-2]["_time"] > 2
        ):
